# tree/nodes/n11_multi_truth_var.py

NODE = {
    "id": "N1_1",
    "title": "Multi-Class Classification: Metrics based on percent agreement and interchangeability",
    "type": "content",
    "content_html": """
     <section style="margin-bottom: 1.5rem;">
       <h2 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 0.5rem; color: #1a202c;">Overview</h2>
       <p style="color: #333; line-height: 1.6;">You have identified your task as a <i>multi-class</i> classification task based on a <i>reference standard with non-negligible unreliability and variability</i>. Currently, there is a lack of widely acknowledged performance evaluation metric used for this context. Please read the following for a summary of evaluation metrics/methods applicable to this scenario.</p>
     </section>
     <section style="margin-bottom: 1.5rem;">
       <h2 style="font-size: 1.5rem; font-weight: bold; margin-bottom: 0.5rem; color: #1a202c;">Evaluation metrics/approaches for multi-class classification with truth variability</h2>
       <p style="color: #dc2626; font-weight: bold; line-height: 1.6;">Highlight message</p>
       <p style="color: #333; line-height: 1.6;">Evaluation of classification models often relies on a perfect ground truth, which may not be available in many real-world scenarios. This leads to truth variability which poses a challenge for evaluating classification tasks. In this discussion, we present three (but not all) approaches &#8212; generalization of percent agreement metrics, interchangeability metric, and latent class analysis &#8212; to address this issue. It is important to note that no universally accepted evaluation metric exists for classification tasks with truth variability, and the presented approaches are intended as starting points to explore this area. <strong>However, it is essential to remember that these methods should not replace the gold standard test, which remains the ideal reference standard for evaluation.</strong></p>
     </section>
     <section style="margin-bottom: 1.5rem;">
       ...
       <p style="color: #333; line-height: 1.6;">Existing studies on multi-class classification evaluation metrics mostly assume a perfect ground truth, with very few studies accounting for truth variability. As a result, there is no consensus approach to evaluation metrics for classification tasks with truth variability. In this summary, we provide an overview of some existing solutions in different categories, with the hope that readers can use this as a starting point to find the most suitable evaluation metric/method for their specific task. <strong>It is important to note that these methods should never be seen as substitutes for a gold standard test, as the gold standard is always the ideal choice (Albert and Dodd, 2004).</strong></p>
     </section>
     <section style="margin-bottom: 1.5rem;">
       <h3 style="font-size: 1.25rem; font-weight: 600; margin-bottom: 0.5rem; color: #1a202c;">Generalization of percent agreement metrics</h3>
       <p style="color: #333; line-height: 1.6;">One approach is to extend the percent agreement metrics, i.e., Positive Percent Agreement (PPA) and Negative Percent Agreement (NPA) (FDA CDRH 2007), from binary classification case, to evaluate the multi-class classifier in comparison to a non-reference standard. The generalized metric, percent agreement for class <i>i</i> is defined as the proportion of non-reference standard subjects in <i>i</i><sup style="font-size: 0.7em;">th</sup> class being classified into <i>i</i><sup style="font-size: 0.7em;">th</sup> class by the algorithm.</p>
       <p style="color: #333; line-height: 1.6;">As the definition implies, percent agreement for class <i>i</i> has the same numerical calculations as its counterpart for class-specific accuracy (see 'balanced accuracy' node under branch "single class label output &gt;&gt; nominal") when the reference standard does not exist.</p>
     </section>
     <section style="margin-bottom: 1.5rem;">
     ...
     <section style="margin-bottom: 1.5rem;">
       ...
       <ul style="list-style-type: disc; margin-left: 1.5rem; color: #333;">
         ...
         <li>Hadgu, A., &amp; Qu, Y. (1998). A biomedical application of latent class models with random effects. Journal of the Royal Statistical Society: Series C (Applied Statistics), 47(4), 603-616.</li>
